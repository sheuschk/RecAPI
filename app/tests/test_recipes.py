...
create_recipe_dto = {"name": "Name", "description": "Desc", "ingredients": {"Ing1": "1", "Ing2": 2}}
update_recipe_dto = {'id': 1, "name": "U_Name", "description": "U_Desc", "ingredients": {"Ing1": "1"}}
...
def test_create_recipe():
    ...
    response = client.post("/recipe/", json=create_recipe_dto)
    assert response.status_code == 200
...
def test_get_recipe():
    ...
    assert response.status_code == 200
    assert response_json == {'id': 1, "name": "Name", "description": "Desc", "ingredients": {"Ing1": "1", "Ing2": 2}}


def test_update_recipe():
    response = client.put("/recipe/", json=update_recipe_dto)
    assert response.status_code == 200
    assert response.json() == {"Msg": "Update successful"}
    get_resp = client.get("/recipe/1")
...
def test_delete_recipe():
    response = client.post("/recipe/1")
    assert response.status_code == 200
    assert response.json() == {"Msg": "Deletion successful"}


def test_update_recipe_does_not_exist():
    response_update = client.put("/recipe/", json=update_recipe_dto)
    response_delete = client.post("/recipe/1")
    response_create = client.get("/recipe/1")
...
def test_recipe_page():
    ...
    for i in range(1, 4):
        ...
        client.post("/recipe/", json=create_recipe_dto)
